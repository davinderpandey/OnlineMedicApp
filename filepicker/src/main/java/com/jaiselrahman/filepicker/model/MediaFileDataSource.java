...
public class MediaFileDataSource extends PositionalDataSource<MediaFile> {
    ...
    /* ArrayList<FileImage> queryScopedStorage()  {
         // Projection is an array that returns what all info we want to get back

         FileImage image =  new  FileImage();
         image.setUri(MediaStore.Images.Media._ID);   // A unique ID of the image, we'll be using this to create our Image URI later
         image.setName(MediaStore.Images.Media.DISPLAY_NAME);
         image.setSize(MediaStore.Images.Media.SIZE);
         image.setDateTaken(MediaStore.Images.Media.DATE_TAKEN);
        ...
        // Sort order that tells that our images should be sorted in the descending order of the date on which they were taken
         ...
         int nameColumn = cursor.getColumnIndexOrThrow(DISPLAY_NAME);
